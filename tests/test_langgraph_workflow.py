    ...
    def test_blocked_contract_can_be_revised_and_resumed(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.harness(root) as harness:
                ...
                self.assertTrue(pending_legal.paused)
                self.assertEqual(pending_legal.waiting_for, "finance_legal_review")
                final = harness.resume(
                    run.case_id,
                    {"action": "approve", "comment": "合同法务审批通过"},
                    actor=ActorContext("legal-1", ("legal",), "web"),
                )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with DongjiangWorkflowHarness(
                ...
                final = harness.resume(
                    run.case_id,
                    {"action": "approve", "comment": "合同法务审批通过"},
                    actor=ActorContext("legal-writeback", ("legal",), "web"),
                )
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with self.harness(root) as second:
                ...
                final = second.resume(
                    case_id,
                    {"action": "approve", "comment": "合同法务审批通过"},
                    actor=ActorContext("legal-2", ("legal",), "web"),
                )
                self.assertFalse(final.paused)
                self.assertEqual(final.state["decision"], "pass")
