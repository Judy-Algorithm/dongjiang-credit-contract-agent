...
案件“Agent运行”页支持对信用资料或合同生成 AI 结构化字段候选。每个候选必须通过字段白名单、Schema、置信度和 `document_id + fragment_id` 证据定位核验；只有信用审批人、合同法务或授权审批人可人工采纳，系统管理员不参与业务审批。生成、拒绝或核验失败均不改变正式字段，采纳也不会自动批准授信或合同。
...
- `POST /api/cases/{case_id}/mock-enterprise-approval`（信用审批人、仅 Mock 模式）
...
- TKP：低/中/高风险建议账期 90/60/30 天；超过 Net 90 天需核验终端项目统一账期或由授权审批人（如市场总监）特别批准。
- TKM：低风险常规条件为最多 40% 尾款、180 天；中高风险进一步收紧，超出常规条件需由授权审批人处理特别信用申请。
...
Web页面包含登录、邮箱验证码注册、邮箱找回密码、通知中心、时效运营、Agent运维、管理分析、我的待办、案件列表、发起信审、七页签案件工作台、案件处理、用户管理和安全审计。首次启动需在页面创建管理员；公开注册必须先验证邮箱，账号创建后立即启用并默认为业务经办人，无需管理员审核，也不会要求首次登录修改密码。管理员仍可在“用户管理”中调整角色、停用账号或重置密码。通知中心还接收案件流转、临期/逾期、Agent异常和失败回写提醒，点击后可进入对应业务页面。案件列表和详情显示当前待办截止时间，管理员可在“时效运营”页查看逾期分布和节点积压，在“Agent运维”页集中查看计划完整性、执行偏差、节点失败/降级、重试和缓存复用，并对异常执行确认、分派、受控候选重跑和关闭，在“管理分析”页查看案件办结率、节点SLA达标率、趋势和责任队列积压，并下载CSV或三工作表Excel报表。业务经办人、信用审批人、合同法务、授权审批人和系统管理员按照真实登录身份执行工作流权限。新案件自动归属发起业务经办人，管理员可以改派负责人。新解析文件支持 PDF 页码、DOCX 段落、XLSX 工作表/单元格和文本行号定位，风险卡片可在受控查看器中打开对应原文。信用资料和合同使用不同入口，合同只能在授信审批生效后上传。上传限制为单文件15MB、单次请求30MB。
...
案件“合同审查”页提供多语言工作台，支持中文、英文、越南语、日语和西班牙语。系统只将带稳定片段 ID 的脱敏文本发送给已配置的文本模型，严格校验片段数量和脱敏令牌；译稿逐段绑定原文，业务经办人或合同法务必须逐段核对、填写复核说明后，才能生成并下载案件级双语 Word。翻译不覆盖原合同、不改变风险结论，也不参与自动批准。
...
比赛演示可在 `.env` 设置 `DONGJIANG_INTEGRATION_MODE=mock`。信用审批人随后可在等待信用审批的案件“系统回写”页点击“运行 Mock OA 审批”，系统会生成带 `mock: true` 标记的完整 OA 审批链、批准范围、有效期和证据 ID，并沿用正式工作流激活授信、执行 OA/CRM/SAP 幂等回写。Mock 记录保存到已忽略的 `data/mock-enterprise/`，只含允许字段摘要；它证明端到端编排可运行，不代表泛微 OA、真实 CRM 或 SAP 已完成联调。生产环境必须清空该模式并配置真实接口。
