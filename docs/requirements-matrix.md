...
| Agent异常自动发现与SLA | 对计划完整性失败、执行偏差、节点失败和连续分析降级自动开单；异常指纹去重；分别跟踪响应/解决时限并发送去重提醒 | `test_agent_incident_automation.py`、`test_agent_incident_sweep_is_admin_only_and_audited` |
| 安全自助注册 | 注册前邮箱验证码；验证后立即启用并分配业务经办人角色；不强制首次登录改密 | `test_verified_registration_is_immediately_active`、注册页 |
| 业务通知中心 | 案件流转、临期/逾期、Agent异常和失败回写通知；按用户隔离，支持单条/全部已读与业务跳转 | `test_notifications_are_scoped_to_recipient_and_support_read_all`、通知中心页 |
